...
import base.models as models_base
import curso_extensao.models as models_curso
...
def tabela_pessoas(doc, cabecalho, table_spec):
    from pylatex import Tabularx, NoEscape
...
def gerar_pdf(curso):
    ...
    with doc.create(Enumerate()) as enum:
        ...
        enum.add_item(bold('TÍTULO: '))
        doc.append(curso.titulo)

        enum.add_item(bold('COORDENADOR(A): '))
        doc.append(curso.coordenador.nome_completo)

        enum.add_item(bold('PERÍODO DE REALIZAÇÃO: '))
        # TODO: formatar data
        doc.append('De ' + str(curso.periodo_realizacao_inicio) + ' a ' + str(curso.periodo_realizacao_fim))
        ...
        for obj in models_base.UnidadeAdministrativa.objects.all():
        ...
        for obj in models_base.Campus.objects.all():
        ...
        doc.append(NewLine())
        for obj in models_base.Centro.objects.all():
        ...
        if curso.grande_area:
            tabela_alternativas(doc, models_base.GrandeArea, '|X|X|X|', id=curso.grande_area.id)

        enum.add_item(bold('PALAVRAS-CHAVE: '))
        doc.append(NewLine())
        tabela_alternativas(doc, models_curso.PalavraChave_CursoExtensao, '|X|X|X|')

        enum.add_item(bold('ÁREA TEMÁTICA PRINCIPAL: '))
        doc.append(NewLine())
        if curso.area_tematica_principal:
            tabela_alternativas(doc, models_base.AreaTematica, '|X|X|X|', id=curso.area_tematica_principal.id)

        enum.add_item(bold('ÁREA TEMÁTICA SECUNDÁRIA: '))
        doc.append(NewLine())
        if curso.area_tematica_secundaria:
            tabela_alternativas(doc, models_base.AreaTematica, '|X|X|X|', id=curso.area_tematica_secundaria.id)

        enum.add_item(bold('LINHA DE EXTENSÃO: '))
        doc.append(NewLine())
        doc.append(NoEscape(r'{\tiny'))
        if curso.linha_extensao:
            tabela_alternativas(doc, models_base.LinhaExtensao, 'X|X|X', id=curso.linha_extensao.id, hline=False)
        doc.append(NoEscape('}'))

        enum.add_item(bold('PÚBLICO ALVO: '))
        doc.append(curso.publico_alvo)

        enum.add_item(bold('NÚMERO DE PESSOAS A SEREM BENEFICIADAS: '))
        doc.append(curso.numero_pessoas_beneficiadas)

        enum.add_item(bold('CARGA HORÁRIA TOTAL: '))
        doc.append(curso.carga_horaria_total)

        enum.add_item(bold('Nº DE VAGAS: '))
        doc.append(curso.numero_vagas)

        enum.add_item(bold('LOCAL DA INSCRIÇÃO: '))
        doc.append(curso.local_inscricao)

        # TODO: calibri 10, justificado (?), espaçamento simples (1,5?)
        enum.add_item(bold('RESUMO: '))
        doc.append(NewLine())
        doc.append(curso.resumo)

        enum.add_item(bold('PROGRAMAÇÃO: '))
        doc.append(curso.programacao)

        enum.add_item(bold('EQUIPE DE TRABALHO: '))
        doc.append(NewLine())
        for servidor in curso.servidores.all():
            servidor_cursoextensao = models_curso.Servidor_CursoExtensao.objects.get(servidor_id=servidor.id)
            ...
            with doc.create(MdFramed(options=mdframed_options)):
                ...
                for obj in models_base.TipoServidor.objects.all():
                ...
                with doc.create(MdFramed(options=mdframed_options)):
                    doc.append('Função: ')
                    doc.append(NewLine())
                    tabela_alternativas(doc, models_base.FuncaoServidor, 'XXX',
        ...
        doc.append(NoEscape('{\scriptsize'))
        tab = tabela_pessoas(doc, cabecalho, table_spec)

        discentes = models_curso.Discente_CursoExtensao.objects.filter(curso_extensao=curso.id)
        ...
        membros = models_curso.MembroComunidade_CursoExtensao.objects.filter(curso_extensao=curso.id)
        ...
        doc.append(NewLine())
        previsao_orcamentaria = models_curso.PrevisaoOrcamentaria_CursoExtensao.objects.get(curso_extensao=curso.id)
        ...
        with doc.create(MdFramed(options=mdframed_options)):
            ...
            for obj in models_base.TipoGestaoRecursosFinanceiros.objects.all():
    ...
    doc.generate_pdf('exemplo-pylatex', clean_tex=False)
